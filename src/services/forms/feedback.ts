// Migrated from src/services/googleForms.ts
// This is the existing feedback form (manually created, entry IDs extracted manually)
// Future forms will be created using the automated create-analytics-form script

import Constants from 'expo-constants';
import * as Device from 'expo-device';

export const FEEDBACK_CONFIG = {
  formUrl: 'https://docs.google.com/forms/u/0/d/e/1FAIpQLSeVcO3N1SQ0MuRelzZtTEFJ0LgfZCSDwyKat_99HSp8t0URtw/formResponse',
  entryIds: {
...
export async function submitFeedback(data: FeedbackData): Promise<void> {
  ...
  formData.append(FEEDBACK_CONFIG.entryIds.feedbackType, data.feedbackType);
  formData.append(FEEDBACK_CONFIG.entryIds.message, data.message);

  // Optional email
  if (data.email && data.email.trim()) {
    formData.append(FEEDBACK_CONFIG.entryIds.email, data.email.trim());
  }

  // Auto-populated context fields
  formData.append(FEEDBACK_CONFIG.entryIds.belt, data.belt);
  formData.append(
    FEEDBACK_CONFIG.entryIds.appVersion,
    Constants.expoConfig?.version || 'unknown'
  );
  formData.append(
    FEEDBACK_CONFIG.entryIds.device,
