...
- Always ask if we need to update `app.json` version before building a new version to be published. Version must increase for App Store/Play Store to accept updates.
...
- **Release workflow rule:** Remind user to update version in `app.json` before building a new release version to be published. Always prompt/ask if `app.json` version needs bumping.
